...
module Attachments
  class UploadMergingServices < ApplicationService
    ...
    private
      ...
      def attach_file
        blob = if Flipper.enabled?(:ft_change_upload_path, @envelope.user)
                 create_blob_from_file(@envelope, merge, 'docs', "#{@envelope.id}.pdf")
               else
                 ActiveStorage::Blob.create_and_upload!(
                   io: merge,
                   filename: "#{@envelope.id}.pdf",
